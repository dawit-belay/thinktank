...
import { ArrowLeft, Lightbulb, PlusCircle, Users } from "lucide-react";
...
import { meetingMembers } from "@/db/schema";
...
export default async function MeetingPage({ params }: MeetingPageProps) {
  ...
  const members = await db.query.meetingMembers.findMany({
  ...
  return (
   <main className="relative min-h-screen overflow-hidden bg-gradient-to-b from-emerald-50/90 via-stone-50 to-zinc-100 px-5 pb-20 pt-10 md:px-8 md:pt-14">
      ...
      <div className="relative z-10 mx-auto max-w-5xl">
        ...
        <section className="mb-8 rounded-3xl border border-zinc-200/80 bg-white/80 p-5 shadow-sm shadow-zinc-200/40 backdrop-blur-sm md:p-6">
          <MeetingHeader
            title={meeting.title}
            stage={meeting.stage}
          ...
          <aside className="space-y-4 lg:sticky lg:top-24 lg:h-fit">
            ...
            {(meeting.stage === "decision" || meeting.stage === "summary") && (
              <DecisionPanel
                meetingId={meetingId}
                groupId={groupId}
                initialDecision={meeting.decisionText ?? ""}
                canManage={!!canManageMeetingMembers}
              />
            )}
